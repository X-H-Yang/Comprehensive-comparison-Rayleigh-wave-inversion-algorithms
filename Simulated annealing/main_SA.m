...
%
% Note: the inversion performance of the SA algorithm for Rayleigh wave 
...
my_fun = @(theta_SA)SAobjfun(theta_SA, layers_num, dispersions_R_true, ...
...
options = saoptimset('MaxIter',3000,... % Maximum number of iterations
'StallIterLimit',500*num_unknown,... % 
'TolFun',1e-6,... % Termination tolerance on function value
'AnnealingFcn',@annealingfast,'InitialTemperature',...
1000,'TemperatureFcn',@temperatureexp,'ReannealInterval',10);
...
tic
[Y_hat,fval,exitflag,output]  = simulannealbnd(my_fun,theta_0,...
