function [y, dispersion_num] = Jg(theta, layers_num, model_dispersion, Vp, den, f,modes_num_vec,index_vec_all,Vs_profile_lower,Vs_profile_upper)
...
if sum(theta-Vs_profile_lower<0) == 0 && sum(theta-Vs_profile_upper>0) == 0
    try
...
%         y = Inf;
        y = 10000 + sum(abs(theta - Vs_profile_lower)./Vs_profile_lower);
    end
else
    temp_1 = sum(abs(theta - Vs_profile_lower)./Vs_profile_lower);
    temp_2 = sum(abs(theta - Vs_profile_upper)./Vs_profile_upper);
    y = 100*(temp_1+temp_2) + 10000;
end
