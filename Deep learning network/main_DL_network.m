...
%
% Note: the inversion performance of the DL network for Rayleigh wave 
...
%% Hyperparameters of the deep learning network
hlayer_vec = 3:2:15;
neuron_vec = 3:2:15;
...
%% Deep learning network for inversion
time_index = zeros(length(hlayer_vec)*length(neuron_vec),1);
Obj_index_temp = zeros(length(hlayer_vec)*length(neuron_vec),1);
Y_hat_all = zeros(length(hlayer_vec)*length(neuron_vec),2*layers_num-1);
result_all = zeros(length(hlayer_vec)*length(neuron_vec),4+2*layers_num-1);

jjj = 0;

disp('Performing inversion ...')
for Num_i= 1:1:length(hlayer_vec)
    for Num_j = 1:1:length(neuron_vec)
        jjj = jjj+1;
        Num_hlayer = hlayer_vec(Num_i);
        Num_neuron = neuron_vec(Num_j);
        my_net_info = [jjj Num_hlayer Num_neuron];
        
        net = fitnet(Num_neuron*ones(1,Num_hlayer),'trainlm');
        net.trainParam.showWindow = 0;
        % train
        [net,tr] = train(net,train_x_norm',train_y_norm');
        Y_hat_norm_temp = net(validation_x_norm');
        time_index(jjj) = toc;
        Y_hat_norm_temp = Y_hat_norm_temp';
        % re-scale to original space
        Y_hat_temp = zeros(size(Y_hat_norm_temp,1),...
            size(Y_hat_norm_temp,2));
        for i = 1:1:size(train_y,2)
            Y_hat_temp(:,i) = (Y_hat_norm_temp(:,i)*std_y(i)) + mean_y(i);
        end
        invertedDispersions = calDispersions_2(Y_hat_temp,Vp_true,...
            den_true,f,modes_num_vec,index_vec_all);
        
        Y_hat_all(jjj,:) = Y_hat_temp;
        Obj_index_temp(jjj) = sqrt(sum((invertedDispersions-...
            validation_x).^2/length(invertedDispersions)));
        
        result_all(jjj,:) = [Num_hlayer Num_neuron time_index(jjj) ...
            Obj_index_temp(jjj) Y_hat_all(jjj,:)];
    end
end
disp('Inversion finished ...')

Obj_index = zeros(size(Obj_index_temp,1),size(Obj_index_temp,2));
temp = 100000; 
for i = 1:1:length(Obj_index_temp)
    error = Obj_index_temp(i);
    if error < temp
        Obj_index(i) = Obj_index_temp(i);
        temp = error;
    else
...
aaa = find(result_all(:,4)==min(result_all(:,4)));
Y_hat = result_all(aaa,5:end);
