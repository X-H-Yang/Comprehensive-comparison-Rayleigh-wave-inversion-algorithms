...
%
% Note: the inversion performance of the PSO algorithm for Rayleigh wave 
...
%% model parameters for dispersion curves
h_true = [2 4 5 5];
h_true2 = [h_true 0];
...
my_fun = @(theta_PSO)PSOobjfun(theta_PSO, layers_num, ...
    dispersions_R_true, Vp, den, f,modes_num_vec,index_vec_all);

num_unknown = length(Vs_profile_lower);

options = optimoptions('particleswarm','CreationFcn', ...
    @pswcreationuniform,'FunctionTolerance',1e-6,'MaxIterations',60,...
    'MinNeighborsFraction',0.25,'SwarmSize',100);
...
tic
[Y_hat,fval,exitflag,output]  = particleswarm(my_fun,num_unknown,...
    Vs_profile_lower,Vs_profile_upper,options);

time_temp_index = time_global_index(:);
Obj_temp_index = sqrt(Obj_global_index);

clear time_global_index Obj_global_index

my_SwarmSize = options.SwarmSize;
time_index = zeros(length(time_temp_index)/my_SwarmSize,1);
Obj_index = time_index;
Obj_temp_index_2 = zeros(length(Obj_temp_index)/my_SwarmSize,1);

for i = 1:length(time_index)
    time_index(i) = time_temp_index(my_SwarmSize*i);
    Obj_temp_index_2(i) = min(Obj_temp_index(...
        my_SwarmSize*i-my_SwarmSize+1:my_SwarmSize*i));
end

for i = 1:length(Obj_temp_index_2)
    Obj_index(i) = min(Obj_temp_index_2(1:i));
end
